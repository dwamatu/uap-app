...
class UserController extends Controller
{
	...
	public function index(Request $request)
	{
  		if ( Shinobi::can( config('watchtower.acl.user.index', false) ) ) {
			if ( $request->has('search_value') ) {
				...
				$users = $this->model->where('firstname', 'LIKE', '%'.$value.'%')
					->orderBy('firstname')->paginate( config('watchtower.pagination.users', 15) );
				session()->flash('search_value', $value);
			} else {
				$users = $this->model->orderBy('firstname')->paginate( config('watchtower.pagination.users', 15) );
	...
	public function showUserMatrix()
	{
  		if ( Shinobi::can( config('watchtower.acl.user.viewmatrix', false) ) ) {
			$roles = Role::all();
			$users = $this->model->orderBy('firstname')->get();
